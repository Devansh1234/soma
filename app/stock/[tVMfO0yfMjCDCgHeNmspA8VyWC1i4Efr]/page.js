...
//
// FILE LOCATION:  app/stock/[token]/page.js  (any [name] works)
// The folder must be wrapped in square brackets — Next.js syntax for a
// dynamic URL segment. The code reads the segment value, not its name.
//
...
export default async function PublicStockPage({ params, searchParams }) {
  ...
  // Read the dynamic URL segment whatever the folder happens to be named
  // ([token], [slug], anything) — we only care about its value.
  const supplied = params ? Object.values(params)[0] : undefined;

  // Env var missing → say so plainly. This reveals nothing secret and makes
  // a misconfiguration obvious instead of looking like a broken link.
  if (!token) {
    return (
      <div style={WRAP}>
        <h1 style={{ fontSize: 20, marginBottom: 8 }}>Viewer not configured</h1>
        <p style={{ fontSize: 14, color: '#666' }}>
          STOCK_PUBLIC_TOKEN is not set on the server.
        </p>
      </div>
    );
  }

  // Wrong token in the URL → behave as if nothing is here
  if (supplied !== token) {
    return (
      <div style={WRAP}>
