...
// Reached at /stock/<token> where <token> must match STOCK_PUBLIC_TOKEN.
//
// Deliberately server-rendered with NO client JavaScript: search is a plain
// HTML GET form. This keeps it working on very old browsers (the showroom
// iPad runs Safari 9, which has no fetch and no modern JS syntax).
//
...
export default async function PublicStockPage({ params, searchParams }) {
  ...
  if (!token || params.token !== token) {
    return (
      <html lang="en">
        <body style={{ fontFamily: 'system-ui, -apple-system, Arial, sans-serif', padding: 40 }}>
  ...
  const rows = data || [];
  const totalUnits = rows.reduce((s, r) => s + (Number(r.available_qty) || 0), 0);
